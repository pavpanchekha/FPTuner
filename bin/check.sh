...
export FPTUNER="${SCRIPT_LOCATION}/fptuner --nightly --cfile"

run ()
{
    \time -f %e ${FPTUNER} ${SCRIPT_LOCATION}/../benchmarks/check_benchmarks/$1 -v medium -e ${@:2} | tee -a ${check_date}/index.html
...
if [ "$(hostname)" = "warfa" ]; then
    scp -r ${check_date} uwplse.org:/var/www/fptuner/
fi
